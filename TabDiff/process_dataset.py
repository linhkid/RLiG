...
def process_data(name):
    ...
    for i, col_idx in enumerate(num_col_idx):
        ...
        # First check if the column is numeric, then check if all values are integers
        try:
            # Convert to numeric, coercing errors to NaN
            numeric_data = pd.to_numeric(col_data, errors='coerce')

            # Check if all values are integers (after removing NaNs)
            is_int = (~pd.isna(numeric_data) & (numeric_data % 1 == 0)).all()
            ...
            # If any exception occurs, this is not an integer column
            pass
